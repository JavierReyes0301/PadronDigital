document.addEventListener("DOMContentLoaded", async function () {
  ...
  // --- 4. LISTENER DE SUPABASE (LA SOLUCIÓN AL CAMBIO DE MENÚ) ---
  if (window.clientSupa) {
...
async function cerrarSesion() {
  try {
    if (window.clientSupa) {
      // 1. Llamada a Supabase para cerrar la sesión en el servidor
      ...
      // 2. Opcional: Limpiar datos locales si usas localStorage extra
      // localStorage.removeItem('tu_variable_personalizada');

      // 3. Redirección lógica
      // Si está en inicio.html, lo mandamos a la landing (index.html)
      if (window.location.pathname.includes("inicio.html")) {
        window.location.href = "index.html";
      } else {
        // Si ya está en la landing, el listener onAuthStateChange
        // se encargará de poner el botón "Acceder" automáticamente.
...
// Aseguramos que la función sea global para que el onclick del menú la encuentre
