document.addEventListener("DOMContentLoaded", async function () {
  // --- 0. INYECCIÓN DE RECURSOS (MANTENIDO IGUAL) ---
  const headContenido = `
  ...
  // --- NUEVA FUNCIÓN: RENDERIZADO REACTIVO ---
  async function renderizarBarra() {
      ...
      if (session) {
        ...
        itemUsuario = `
          <li class="nav-item dropdown">
            <a class="nav-link-item dropdown-toggle" href="javascript:void(0);" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true">
    ...
    if (navPlaceholder) {
      ...
      // Re-vincular el Toggle Mobile cada vez que se redibuja
  ...
  // --- ESCUCHADOR DE SUPABASE (LA ADECUACIÓN) ---
  if (window.clientSupa) {
    window.clientSupa.auth.onAuthStateChange(() => {
      renderizarBarra();
    });
  }

  // --- EJECUCIÓN INICIAL ---
  await renderizarBarra();

  // El resto de tu código (Footer, Redirects, Scrollspy, Detección Inicial) se queda EXACTAMENTE igual
  const footerPlaceholder = document.getElementById("footer-placeholder");
  if (footerPlaceholder) {
    footerPlaceholder.innerHTML = `<footer class="mi-footer"><div class="container-fluid"><p class="mb-0 text-center">© 2026 H. Ayuntamiento de Atlixco. Todos los derechos reservados.</p></div></footer>`;
  }
...
// --- FUNCIONES GLOBALES (MANTENIDAS IGUAL) ---
